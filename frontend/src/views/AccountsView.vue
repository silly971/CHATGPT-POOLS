...
import { API_URL, authService, gptAccountService, openaiOAuthService, userService, type AccountStatus, type CheckAccountStatusItem, type CheckAccountStatusResponse, type GptAccount, type CreateGptAccountDto, type SyncUserCountResponse, type GptAccountsListParams, type ChatgptAccountInviteItem, type ChatgptAccountCheckInfo, type CheckGptAccessTokenQuota, type OpenAIOAuthSession, type OpenAIOAuthExchangeResult } from '@/services/api'
...
onMounted(async () => {
  ...
  window.addEventListener('message', handleOpenaiOAuthMessage)
  ...
  await loadAccounts()
  await consumePendingOpenaiOAuthResult()
})

onUnmounted(() => {
  window.removeEventListener('message', handleOpenaiOAuthMessage)
...
const checkedQuota = ref<CheckGptAccessTokenQuota | null>(null)
...
const applyOpenaiOAuthResult = async (
  result: OpenAIOAuthExchangeResult,
  options: { ensureDialog?: boolean; showToast?: boolean } = {}
) => {
  const ensureDialog = options.ensureDialog !== false
  const showToast = options.showToast !== false

  if (ensureDialog && !showDialog.value) {
  ...
  const oauthPlanType = String(result?.accountInfo?.planType || '').trim().toLowerCase()
  ...
  if (oauthPlanType) {
    formData.value.planType = oauthPlanType
    formData.value.accountType = normalizeAccountType(formData.value.accountType, oauthPlanType)
  }
  ...
  if (accessToken) {
    ...
    try {
      ...
      checkedQuota.value = checked?.quota || null
...
const consumePendingOpenaiOAuthResult = async () => {
  const raw = sessionStorage.getItem(OPENAI_OAUTH_RESULT_STORAGE_KEY)
  if (!raw) return
  sessionStorage.removeItem(OPENAI_OAUTH_RESULT_STORAGE_KEY)

  try {
    const parsed = JSON.parse(raw)
    if (parsed?.type !== OPENAI_OAUTH_MESSAGE_TYPE || !parsed?.result) return
    await applyOpenaiOAuthResult(parsed.result as OpenAIOAuthExchangeResult, { ensureDialog: true, showToast: true })
  } catch (error) {
    console.warn('Failed to consume OpenAI OAuth callback result', error)
  }
}

const handleOpenaiOAuthMessage = async (event: MessageEvent) => {
  if (event.origin !== window.location.origin) return
  const payload = event.data
  if (!payload || payload.type !== OPENAI_OAUTH_MESSAGE_TYPE || !payload.result) return
  try {
    await applyOpenaiOAuthResult(payload.result as OpenAIOAuthExchangeResult, { ensureDialog: true, showToast: true })
  } catch (error) {
    console.error('Apply OpenAI OAuth result from message failed', error)
  }
}
...
const handleExchangeOpenaiCode = async () => {
  ...
  const state = extractOAuthState(openaiOAuthInput.value) || fallbackState
  if (!code) {
    showErrorToast('请粘贴回调 URL 或授权码（code）')
    return
  }
  if (!state) {
    showErrorToast('缺少 state，请重新点击“获取”生成授权链接')
    return
  }
  ...
  try {
    ...
    openaiOAuthError.value = ''
    const result = await openaiOAuthService.exchangeCode({ code, state, sessionId })
    if (currentNonce !== openaiOAuthFlowNonce) return
    await applyOpenaiOAuthResult(result, { ensureDialog: false, showToast: true })
  } catch (err: any) {
    if (currentNonce !== openaiOAuthFlowNonce) return
...
type NormalizedAccountType = 'team' | 'personal' | 'unknown'
const normalizeAccountType = (value: unknown, planType?: string | null): NormalizedAccountType => {
  const normalized = String(value || '').trim().toLowerCase()
  if (normalized === 'team' || normalized === 'personal' || normalized === 'unknown') {
    return normalized
  }
  const normalizedPlanType = String(planType || '').trim().toLowerCase()
  if (normalizedPlanType === 'team') return 'team'
  if (normalizedPlanType) return 'personal'
  return 'unknown'
}

const resolveAccountType = (account?: { accountType?: string; planType?: string | null } | null): NormalizedAccountType => {
  if (!account) return 'unknown'
  return normalizeAccountType(account.accountType, account.planType || null)
}

const getAccountTypeBadge = (account?: { accountType?: string; planType?: string | null } | null) => {
  const type = resolveAccountType(account)
  if (type === 'team') {
    return { type, label: 'Team', class: 'bg-blue-50 text-blue-700 border-blue-200' }
  }
  if (type === 'personal') {
    return { type, label: '个人', class: 'bg-amber-50 text-amber-700 border-amber-200' }
  }
  return { type, label: '未知', class: 'bg-gray-50 text-gray-700 border-gray-200' }
}

const getTeamOperationBlockedReason = (account?: { accountType?: string; planType?: string | null } | null): string => {
  const type = resolveAccountType(account)
  if (type === 'personal') return '个人账号不支持 Team 成员操作'
  return ''
}

const syncResultTeamBlockedReason = computed(() => {
  if (!syncResult.value?.account) return ''
  return getTeamOperationBlockedReason(syncResult.value.account)
})
...
const applyCheckedAccountSelection = (accountId: string) => {
  ...
  if (!normalized && !checkedQuota.value) return

  const matched = checkedChatgptAccounts.value.find(acc => acc.accountId === normalized)

  if (matched) {
    formData.value.accountType = normalizeAccountType(matched.accountType, matched.planType)
    formData.value.planType = matched.planType || ''
  }

  const preferredExpiresAt = checkedQuota.value?.expiresAt || matched?.expiresAt || ''
  if (preferredExpiresAt) {
    const localValue = isoToDatetimeLocal(preferredExpiresAt)
    if (localValue) {
      formData.value.expireAt = localValue
    }
  }

  if (!matched && checkedQuota.value) {
    formData.value.accountType = normalizeAccountType(checkedQuota.value.accountType, checkedQuota.value.planType)
    formData.value.planType = checkedQuota.value.planType || formData.value.planType || ''
  }
}
...
const handleCheckAccessToken = async () => {
  ...
  try {
    ...
    checkedQuota.value = result?.quota || null

    if (!checkedChatgptAccounts.value.length && !checkedQuota.value) {
      showWarningToast('校验成功，但未返回可用额度信息，请手动填写 ChatGPT ID')
      return
    }

    const personalAccount = checkedChatgptAccounts.value.find(acc => resolveAccountType(acc) === 'personal')
    const currentAccountId = String(formData.value.chatgptAccountId || '').trim()
    if (!currentAccountId) {
      const fallbackId =
        personalAccount?.accountId ||
        String(checkedQuota.value?.sourceAccountId || '').trim() ||
        (checkedChatgptAccounts.value.length === 1 ? String(checkedChatgptAccounts.value[0]?.accountId || '').trim() : '')
      if (fallbackId) {
        formData.value.chatgptAccountId = fallbackId
      }
    }
    ...
    const accountCountText = `账号 ${checkedChatgptAccounts.value.length} 个`
    const quotaStatusText = checkedQuota.value ? `，额度状态 ${checkedQuota.value.status}` : ''
    showSuccessToast(`校验成功：${accountCountText}${quotaStatusText}`)

    if (resolveAccountType({ accountType: formData.value.accountType, planType: formData.value.planType || null }) === 'personal') {
      showInfoToast('个人账号可保存，但不支持邀请和成员管理')
    }
  } catch (err: any) {
...
const handleSubmit = async () => {
  try {
    const normalizedPlanType = String(formData.value.planType || '').trim().toLowerCase()
    const payload: CreateGptAccountDto = {
      ...
      accountType: normalizeAccountType(formData.value.accountType, normalizedPlanType),
...
const handleSyncUserCount = async (account: GptAccount) => {
  const blockedReason = getTeamOperationBlockedReason(account)
  if (blockedReason) {
    showWarningToast(blockedReason)
    return
  }
  ...
  try {
    ...
    const message = err.response?.data?.error || '同步失败，请检查网络连接和账号配置'
    if (message.includes('个人账号不支持 Team 成员操作') || message.includes('未识别为 Team')) {
      showErrorToast(message)
      await loadAccounts()
      return
    }
...
const handleDeleteSyncedUser = async (userId?: string) => {
  ...
  if (syncResultTeamBlockedReason.value) {
    showWarningToast(syncResultTeamBlockedReason.value)
    return
  }
	                  ...
	                  <div class="flex items-center gap-3">
	                    <div class="w-8 h-8 rounded-full bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center text-xs font-bold text-gray-600">
                          ...
                          <span
                            ...
                            :class="getAccountTypeBadge(account).class"
                          >
                            {{ getAccountTypeBadge(account).label }}
                          </span>
	                        </div>
	                      </div>
		                  </div>
           ...
           <template v-else>
                    ...
                    <!-- Sync -->
	                    <Button 
	                      ...
	                      variant="ghost"
                      class="h-8 w-8 rounded-lg"
                      :class="getTeamOperationBlockedReason(account) ? 'text-gray-300 cursor-not-allowed' : 'text-gray-400 hover:text-blue-600 hover:bg-blue-50'"
	                      @click="handleSyncUserCount(account)"
	                      :disabled="syncingAccountId === account.id || Boolean(getTeamOperationBlockedReason(account))"
	                      :title="getTeamOperationBlockedReason(account) || '同步数据'"
	                    >
            ...
            <div class="flex items-center justify-between gap-2 pt-2 border-t border-gray-50">
               ...
               <div class="flex gap-1">
                  <Button
                    ...
                    variant="ghost"
                    class="h-9 w-9"
                    :class="getTeamOperationBlockedReason(account) ? 'text-gray-300 cursor-not-allowed' : 'text-gray-400'"
                    @click="handleSyncUserCount(account)"
                    :disabled="syncingAccountId === account.id || Boolean(getTeamOperationBlockedReason(account))"
                    :title="getTeamOperationBlockedReason(account) || '同步数据'"
                  >
        ...
        <form @submit.prevent="handleSubmit" class="flex-1 min-h-0 flex flex-col">
           <div class="flex-1 min-h-0 px-8 pb-6 space-y-4 overflow-y-auto">
              ...
              <div class="space-y-2">
                ...
                <p v-else-if="checkedChatgptAccounts.length || checkedQuota" class="text-[12px] text-gray-400">
                  已获取 {{ checkedChatgptAccounts.length }} 个账号
                  <span v-if="checkedQuota">
                    ，额度状态：{{ checkedQuota.status }}
                    <span v-if="checkedQuota.expiresAt">，到期：{{ checkedQuota.expiresAt }}</span>
                  </span>
                </p>
                <p
                  v-if="resolveAccountType({ accountType: formData.accountType, planType: formData.planType || null }) === 'personal'"
                  class="text-[12px] text-amber-600"
                >
                  当前为个人账号：可保存使用，但不支持邀请和成员管理。
                </p>
              </div>
		              ...
		              <div class="grid grid-cols-2 gap-4">
                        ...
                        <datalist v-if="checkedChatgptAccounts.length" id="chatgpt-account-id-options">
                          <option
                            ...
                            :value="acc.accountId"
                          >
                            {{ acc.name }}（{{ resolveAccountType(acc) === 'team' ? 'Team' : resolveAccountType(acc) === 'personal' ? '个人' : '未知' }}）{{ acc.expiresAt ? ` (到期 ${acc.expiresAt})` : '' }}
                          </option>
                        </datalist>
		                 </div>
		                 <div class="space-y-2">
           ...
           <div class="flex-1 overflow-y-auto p-8">
              ...
              <div class="space-y-4">
                 ...
                 >
                      ...
                      :disabled="Boolean(syncResultTeamBlockedReason)"
                      :title="syncResultTeamBlockedReason || '邀请新成员'"
                    >
	                       {{ showInviteForm ? '取消' : '邀请新成员' }}
	                    </Button>
	                 </div>
                 <p v-if="syncResultTeamBlockedReason" class="text-[12px] text-amber-600">
                    {{ syncResultTeamBlockedReason }}
                 </p>
	                 ...
	                 <div v-if="showInviteForm" class="p-4 bg-blue-50/50 rounded-xl border border-blue-100 animate-in fade-in slide-in-from-top-2">
	                    <div class="flex gap-2">
	                       <Input v-model="inviteEmail" placeholder="输入邮箱地址..." class="bg-white h-10 border-blue-200 focus:border-blue-400" :disabled="Boolean(syncResultTeamBlockedReason)" />
	                       <Button @click="handleInviteSubmit" :disabled="inviting || Boolean(syncResultTeamBlockedReason)" class="bg-blue-600 hover:bg-blue-700 text-white h-10 px-4 whitespace-nowrap">
	                          发送邀请
	                       </Button>
	                    </div>
	                 </div>
                          ...
                          <div class="font-medium text-gray-900">{{ user.name }}</div>
                             ...
                             <td class="px-4 py-3 text-right">
	                                <button 
	                                   ...
	                                   :disabled="Boolean(syncResultTeamBlockedReason) || deletingUserId === (user.id || user.account_user_id)"
	                                >
                                   <Trash2 class="w-4 h-4" />
              </button>
                             </td>
                          ...
                          <tr v-for="invite in invitesList" :key="invite.id" class="group hover:bg-gray-50/50">
                             ...
                             <td class="px-4 py-3 text-right">
	                                <button
	                                  ...
	                                  :disabled="Boolean(syncResultTeamBlockedReason) || !invite.email_address || revokingInviteEmail === String(invite.email_address).trim().toLowerCase()"
	                                  title="撤回邀请"
	                                >
