...
export interface GptAccount {
  ...
  accountType?: GptAccountType
  planType?: string | null
  createdAt: string
  updatedAt: string
}

export type GptAccountType = 'team' | 'personal' | 'unknown'
export type GptQuotaStatus = 'normal' | 'expired' | 'unknown'
...
export interface CheckGptAccessTokenQuota {
  sourceAccountId: string
  planType: string | null
  accountType?: GptAccountType
  expiresAt: string | null
  hasActiveSubscription: boolean
  status: GptQuotaStatus
}

export interface CheckGptAccessTokenResponse {
  accounts: ChatgptAccountCheckInfo[]
  quota?: CheckGptAccessTokenQuota | null
}
...
export const openaiOAuthService = {
  async generateAuthUrl(apiKey: string, payload?: { proxy?: string; redirectUri?: string }): Promise<OpenAIOAuthSession> {
  ...
  async exchangeCode(payload: { code: string; state: string; sessionId?: string }): Promise<OpenAIOAuthExchangeResult> {
