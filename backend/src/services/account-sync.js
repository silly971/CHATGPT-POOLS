...
const inferAccountTypeFromPlanType = (planType) => {
  const normalized = normalizePlanTypeValue(planType)
  if (!normalized) return 'unknown'
  if (normalized === 'team') return 'team'
  return 'personal'
}

const normalizeAccountTypeValue = (value, fallback = 'unknown') => {
  const normalized = String(value ?? '').trim().toLowerCase()
...
function mapRowToAccount(row) {
  return {
    ...
    createdAt: row[11],
    updatedAt: row[12],
    accountType: normalizeAccountTypeValue(row[13], 'unknown'),
    planType: normalizePlanTypeValue(row[14])
  }
}
...
export async function fetchOpenAiAccountInfo(token, proxy = null) {
  ...
  if (status < 200 || status >= 300) {
    ...
    const upstreamCode = String(parsedError?.error?.code || parsedError?.code || '').trim()
    if (status === 401) {
      if (upstreamCode === 'account_deactivated') {
        throw new AccountSyncError('OpenAI 账号已停用（account_deactivated）', 401)
      }
  ...
  const orderedKeys = [...orderedIds, ...fallbackIds].filter(id => Boolean(id))
  ...
  for (const key of orderedKeys) {
    ...
    const normalizedAccountId = candidateIds
    ...
    const planType = normalizePlanTypeValue(acc?.account?.plan_type || acc?.plan_type || null)
    const accountType = inferAccountTypeFromPlanType(planType)
    normalizedAccounts.push({
      accountId: normalizedAccountId,
      name: acc?.account?.name || acc?.name || (accountType === 'team' ? 'Unnamed Team' : 'Unnamed Account'),
      planType,
      accountType,
...
export function selectQuotaSourceFromAccounts(accounts, nowMs = Date.now()) {
  const list = Array.isArray(accounts) ? accounts : []
  if (!list.length) return null

  const normalized = list.map(account => ({
    accountId: String(account?.accountId || '').trim(),
    planType: normalizePlanTypeValue(account?.planType),
    accountType: normalizeAccountTypeValue(account?.accountType, inferAccountTypeFromPlanType(account?.planType)),
    expiresAt: account?.expiresAt || null,
    hasActiveSubscription: Boolean(account?.hasActiveSubscription)
  }))

  const findByType = (type) => normalized.find(item => item.accountType === type && item.accountId)
  const selected = findByType('personal') || findByType('team') || normalized.find(item => item.accountId)
  if (!selected) return null

  const expiresAtMs = parseTimestampToMs(selected.expiresAt)
  let status = 'unknown'
  if (expiresAtMs != null) {
    status = expiresAtMs > nowMs ? 'normal' : 'expired'
  } else if (selected.hasActiveSubscription) {
    status = 'normal'
  }
