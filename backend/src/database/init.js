        ...
        try {
          ...
          if (tableInfo.length > 0) {
            ...
            if (!columns.includes('account_type')) {
              database.run(`ALTER TABLE gpt_accounts ADD COLUMN account_type TEXT DEFAULT 'unknown'`)
              console.log('已添加 account_type 列到 gpt_accounts 表')
              saveDatabase()
            }

            if (!columns.includes('plan_type')) {
              database.run('ALTER TABLE gpt_accounts ADD COLUMN plan_type TEXT')
